...
define(['fs', 'path', 'typescript/lib/tsserverlibrary', 'vscode-languageserver', 'vscode-jsonrpc', 'vscode-uri'], function (fs, path, tsserverlibrary, vscodeLanguageserver, vscodeJsonrpc, vscodeUri) { 'use strict';
  ...
  `;
	...
	class Session {
	    ...
	    addProtocolHandlers(conn) {
	        ...
	        conn.onNotification(notifications.NgccComplete, p => this.handleNgccNotification(p));
	    }
	    handleNgccNotification(params) {
	    ...
	    onDidOpenTextDocument(params) {
	        ...
	        try {
	            ...
	            const project = configFileName ?
	                this.projectService.findProject(configFileName) : (_a = this.projectService.getScriptInfo(filePath)) === null || _a === void 0 ? void 0 : _a.containingProjects.find(isConfiguredProject);
	            if (!project) {
	    ...
	    getLSAndScriptInfo(textDocumentOrFileName) {
	        const filePath = vscodeLanguageserver__default['default'].TextDocumentIdentifier.is(textDocumentOrFileName) ?
	    ...
	    checkProject(project) {
	        ...
	        if (this.ivy && project instanceof tsserverlibrary__default['default'].server.ConfiguredProject) {
	            // Keep language service disabled until ngcc is completed.
	            project.disableLanguageService();
	            this.connection.sendNotification(notifications.RunNgcc, {
